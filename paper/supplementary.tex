...
A preliminary probe scaled five global mechanism classes and evaluated the raw finite-difference Hessian rather than its Gauss--Newton form. The per-parameter curvature magnitudes ordered the classes as synaptic $331.97$, gap-junction $299.64$, motor-output $270.50$ (stiff) against ion-channel and passive-membrane $\approx0$ (sloppy), but the raw Hessian at that point carried mixed-sign eigenvalues ($+348$, $-340$, $-214$, $0$, $0$), indicating a saddle rather than a minimum. Curvature signs are therefore not interpretable at that point, and we do not use this probe for any reported result. It is superseded by the positive semi-definite Gauss--Newton decomposition over $20$ named mechanisms (\S5.2), which resolves the same classes at finer granularity, and by the Gauss--Newton analyses of every other system reported here. We retain it only to document the ordering it produced, which agrees with the Gauss--Newton result. File: \texttt{zerocompute\_crosscheck.json}.
...
\begin{figure}[H]\centering
\includegraphics[width=0.47\textwidth]{fig_p2_flyvis_b1.png}\hfill
\includegraphics[width=0.47\textwidth]{fig_p2_crosssystem_real.png}
\caption{	extbf{Stiff and sloppy structure in a connectome-constrained visual model and in the crustacean pyloric population.} 	extbf{a} Behavioural elasticity of the flyvis optic-lobe model across its $65$ named cell types, showing the ten stiffest above the dashed line and the ten sloppiest below. Blue: stiff; grey: sloppy. The elementary motion detectors T4a--d and their medulla inputs are stiff, whereas photoreceptors and lamina wide-field cells are sloppy, and the effective dimension is $43$ of $65$. 	extbf{b} Eigenvector loadings of the rhythm-gated Prinz--Marder pyloric population, with the six largest stiff loadings above the dashed line and the six largest sloppy loadings below. Stiff, leak and inhibitory-synapse conductances; sloppy, fast voltage-gated sodium; effective dimension $2.13$ of $31$ over $K{=}8$ valid points. Note that the biophysical split is the same in both systems and in the worm models of the main text (connectivity and slow or passive properties stiff, fast voltage-gated inward currents sloppy), while the effective dimension is not: a precision visual code constrains many more directions than a whole-body motor pattern. Both probes use the identical Gauss--Newton method and the same finite-difference step.}
...
On the real modWorm model, the union effective dimension over a behaviour set grows with the number of behaviours and then saturates: across twelve behaviours the union reaches $4$ ($99\%$) / $2$ ($90\%$) of $7$, leaving a residual $2$--$3$ directions that no behaviour constrains. Four genuinely distinct locomotor presets (gentle/harsh $\times$ anterior/posterior touch) each constrain $2$--$4$ of $7$ mechanism directions, their leading stiff subspaces only partially overlapping (mean pairwise $\cos\,0.54$, range $0.35$--$0.77$), and their union is $5$ ($99\%$) / $2$ ($90\%$). Quantifying the complementarity directly: the four behaviour-specific stiff directions span an effective $2.98$ of $4$ dimensions (a single shared direction would give $1$; the participation dimension of their singular values), and each behaviour contributes a mean $0.535$ component orthogonal to the span of the other three (per-behaviour $0.56$/$0.65$/$0.46$/$0.47$). To establish that this growth is not the trivial consequence of non-identical subspaces, we compare the observed geometry against two explicit nulls, with $20{,}000$ draws each. Under a \emph{redundant} null, in which every behaviour constrains one shared direction, the span would be $1$, the novel fraction $0$ and the mean pairwise cosine $1$. Under a \emph{random-orientation} null, with directions drawn uniformly on the sphere in the same ambient space, the span is $3.50$ ($95\%$ interval $3.00$--$3.88$), the novel fraction $0.735$ and the mean pairwise cosine $0.312$. The observed values (span $2.978$, novel fraction $0.535$, cosine $0.525$) lie between the two and are significantly more aligned than random ($p{=}0.020$ for the span, $p{=}0.082$ for the novel fraction, $p{=}0.016$ for the cosine). The behaviours therefore share substantial structure while each still contributes a distinct component. With only four behaviours, a bootstrap over behaviours is weak---resampling can draw duplicates and collapse the span---so we report the null comparison as the primary statistic and the bootstrap interval (span $1.45$--$2.98$) only for completeness. File \texttt{null\_model\_tiling.json}. The residual core is genuine biological degeneracy, not an unused parameter (S6.7). Files \texttt{SAT\_saturation.json}, \texttt{MB\_behaviour\_specificity.json}.
