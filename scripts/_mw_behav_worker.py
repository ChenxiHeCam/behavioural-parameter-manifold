"""modWorm behaviour worker: one rollout under per-mechanism scalings AND a chosen stimulus
(different stimuli -> different behaviours: anterior touch=reversal, posterior=forward escape).
Returns the 6 world-frame locomotion observables. argv1 JSON: {gap..B, stim, n_steps}."""
...
sp = json.loads(sys.argv[1]); NSTEP = int(sp.get('n_steps', 150)); STIM = sp.get('stim', 'input_mat_gentle_post_touch')
CE = nps.CE
...
stim = np.load(f'/root/autodl-tmp/modWorm_fresh/modWorm/presets_input/{STIM}.npy') * sp.get('stim_scale', 1.0)
...
obs = {'net_disp': float(np.sqrt((cx[-1] - cx[0]) ** 2 + (cy[-1] - cy[0]) ** 2)),
       ...
       'rms_heading': float(np.sqrt((dh ** 2).mean())) if dh.size else 0.0,
       'final_curve': float(np.abs(dh).sum()) if dh.size else 0.0}
print('RESULT ' + json.dumps({'obs': obs}))
